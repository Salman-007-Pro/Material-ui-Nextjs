...
const Estimate = ({ setValue, setSelectedIndex }) => {
  ...
  const slideQuestion = (val) => {
    const newQuestions = cloneDeep(questions);
    const currentlyActive = newQuestions.filter((question) => question.active);
    const activeIndex = currentlyActive[0].id - 1;
    const nextIndex = activeIndex + val;
    newQuestions[activeIndex] = { ...currentlyActive[0], active: false };
    newQuestions[nextIndex] = { ...newQuestions[nextIndex], active: true };
    setQuestions(newQuestions);
  };
  const checkButtonDisable = (direction) => {
    const currentlyActive = questions.filter((question) => question.active);
    const activeIndex = currentlyActive[0].id - 1;
    const lastIndex = questions[questions.length - 1].id - 1;
    switch (direction) {
      case "forward":
        if (activeIndex === lastIndex) return true;
        else return false;
      case "backward":
    ...
    <Grid container direction="row">
      ...
      >
        ...
        <Grid
          item
          style={{
            width: matchesSM ? "15rem" : matchesMD ? "18rem" : "23rem",
            marginTop: "2rem",
          }}
        >
          <Grid container justify="space-between">
            <Grid item>
              <IconButton
                onClick={() => slideQuestion(-1)}
                disabled={checkButtonDisable("backward")}
              >
                {checkButtonDisable("backward") ? (
                  <img
            ...
            <Grid item>
              <IconButton
                onClick={() => slideQuestion(1)}
                disabled={checkButtonDisable("forward")}
              >
                {checkButtonDisable("forward") ? (
                  <img
                    src="/assets/forwardArrowDisabled.svg"
                    alt="Next question"
                    style={{ verticalAlign: "middle" }}
                  />
                ) : (
        ...
        <Grid item align="center">
          <Button
            classes={{ root: classes.estimateButton }}
